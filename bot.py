...
ms = MovieSaver(cred_path=gsheets_cred, tmdb_api_key=config['tmdb_api_key'], proxy=proxy, note_db_path=config['note_db_path'])
...
def callback_query(call):
    if call.data == "save_note":
        ...
        if len(bot.movies) == 0:
            ...
            os.path.exists('tmp.jpg') and os.remove('tmp.jpg')
            ms.download_poster(info.get('poster_path') or movie.get('poster_path'), out_path='tmp.jpg')
            if os.path.exists('tmp.jpg'):
                ...
                msg = bot.send_message(bot.chat_id, description, reply_markup=check_movie_markup())
            bot.to_delete.append(msg.message_id)
